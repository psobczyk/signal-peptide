...
uruchom_model <- function(probka){
  ...
  rModels <- list(
    list(
      GLMresponse(formula=probka~1, family=multinomial(), pstart=state1, fixed=c(TRUE, TRUE, TRUE, TRUE, TRUE)[1:liczba_odpowiedzi])
    ),
    list(
      GLMresponse(formula=probka~1, family=multinomial(), pstart=state2, fixed=c(TRUE, TRUE, TRUE, TRUE, TRUE)[1:liczba_odpowiedzi])
    ),
    list(
      GLMresponse(formula=probka~1, family=multinomial(), pstart=state3, fixed=c(TRUE, TRUE, TRUE, TRUE, TRUE)[1:liczba_odpowiedzi])
    )
  )
  ...
  transition[[2]] <- transInit(~1,nstates=numStates, family=multinomial("identity"), pstart=c(0, 0.92, 0.08), fixed=c(TRUE,TRUE,TRUE))
