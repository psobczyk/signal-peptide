...
plot(c(0, 1.7), c(0, 1), cex = 0, axes = FALSE, xlab = "", ylab = "",
     mar=c(0.1, 0.1, 2.1, 0.1), main = "HSMM model layout")
...
print(tabAminokwasow(aa5, nazwy2, caption = "Classification of amino acids using only polarity and charge of a side chain."))
@
\end{frame}
...
<<echo = FALSE,message=FALSE,results='hide', fig.height=4.5>>=
plot(density(sapply(hundred_reps, function(i) i$chars$auc)), main = "Density of AUC")
lines(density(sapply(hundred_reps2, function(i) i$chars$auc)), col = "red")
...
all_cs2 <- do.call("rbind", lapply(hundred_reps2, function(i) i[["cs"]]))
